/* ─── Vector Puppet System ───
   SVG-based bone rigging for 2D character animation.
   Each puppet = hierarchical SVG groups with CSS rotate transforms.
*/
...
export const averageMalePuppet: PuppetTemplate = {
  ...
  parts: [
    ...
    // ── Torso ──
    // Main body with rounded shoulders and taper
...
/* ─── Puppet registry ─── */
export const puppetTemplates: PuppetTemplate[] = [
  averageMalePuppet,
  averageFemalePuppet,
  childPuppet,
];
...
export const puppetMotionPresets: MotionPreset[] = [
  ...
  },
  {
    ...
    keyframes: {
      ...
      leftForeArm: [kf(0, -70)],
      rightArm: [kf(0, 30)],
      rightForeArm: [kf(0, -70)],
      leftLeg: [kf(0, -90)],
      leftLowerLeg: [kf(0, 90)],
      rightLeg: [kf(0, -90)],
      rightLowerLeg: [kf(0, 90)],
    },
  },
  {
    id: 'm_happy',
    name: 'happy',
    displayName: 'Happy / Cheer',
    category: 'combined',
    durationSec: 1.5,
    fps: 12,
    loop: true,
...
export function computeBoneAngles(
  preset: MotionPreset,
  elapsedMs: number,
  isLooping: boolean
...
/** Convert text into a stream of visemes for lip-sync */
export function textToVisemes(text: string): Viseme[] {
  const cleaned = text.toLowerCase().replace(/[^a-z]/g, '');
  const result: Viseme[] = [];
  // Process in chunks of ~2 chars = one syllable beat
  for (let i = 0; i < cleaned.length; i += 2) {
    const chunk = cleaned.slice(i, i + 2);
    // Prefer vowel as dominant viseme
...
/** Get the viseme that should be active at a given elapsed time */
...
/** Per-layer deterministic eye-blink using layerId as seed */
export function isEyeOpenAtTime(layerId: string, elapsedMs: number): boolean {
  ...
  const interval = 2500 + (Math.abs(hash) % 2000); // 2.5-4.5s between blinks
  const blinkDur = 120;
  const cycle = interval + blinkDur;
  const phase = elapsedMs % cycle;
  return phase < interval; // true = open, false = closed
}
